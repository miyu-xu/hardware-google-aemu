...
namespace {
    ...
    unsigned long long get_file_index(const std::wstring& path) {
        // Use a unique_ptr to manage the HANDLE
        std::unique_ptr<void, decltype(&::CloseHandle)> file(
        ...
        return (static_cast<unsigned long long>(info.nFileIndexHigh) << 32) | info.nFileIndexLow;
    }
    ...
    std::wstring utf8_to_wide(const std::string& input) {
        ...
        if (MultiByteToWideChar(CP_UTF8, 0, input.c_str(), -1, &output[0], size_needed) <= 0) {
            return std::wstring();
        }

        // Remove the null terminator since std::wstring manages its own
        output.pop_back();
        return output;
    }
    ...
    std::string wide_to_utf8(const std::wstring& input) {
        ...
        if (WideCharToMultiByte(CP_UTF8, 0, input.c_str(), -1, &output[0], size_needed, nullptr, nullptr) <= 0) {
            return std::string();
        }

        // Remove the null terminator since std::string manages its own
        output.pop_back();
        return output;
    }
...
struct InternalDir {
    ...
    // Constructor
    InternalDir() : handle(INVALID_HANDLE_VALUE), first(true), end_reached(false), current_position(0) {
