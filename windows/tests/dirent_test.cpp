   ...
   protected:
    ...
    void TearDown() override {
        try {
            ...
            std::cerr << "Warnig failed to remove directory: " << e.what() << std::endl;
        }
    }
...
TEST_F(DirentTest, ReadDirBasic) {
    createFile(tempDir.string() + "/file1.txt");
    createFile(tempDir.string() + "/file2.txt");
...
TEST_F(DirentTest, ErrorConditions) {
    ...
    // Not a directory
    createFile(tempDir.string() + "/file.txt");
    dir = opendir((tempDir.string() + "/file.txt").c_str());
    ASSERT_EQ(nullptr, dir);
