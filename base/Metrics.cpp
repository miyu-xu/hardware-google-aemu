...
constexpr int64_t kEmulatorGraphicsBadPacketLen = 10031;
constexpr int64_t kEmulatorGraphicsDuplicateSeqno = 10032;
...
struct MetricTypeVisitor {
    ...
    void operator()(const MetricEventBadPacketLen badPacketLenEvent) const {
        if (MetricsLogger::add_instant_event_with_metric_callback) {
            MetricsLogger::add_instant_event_with_metric_callback(kEmulatorGraphicsBadPacketLen,
                                                                  badPacketLenEvent.len);
        }
    }

    void operator()(const MetricEventDuplicateGfxSeqno duplicateGfxSeqnoEvent) const {
        if (MetricsLogger::add_instant_event_with_descriptor_callback) {
            MetricsLogger::add_instant_event_with_descriptor_callback(kEmulatorGraphicsDuplicateSeqno,
                                                                      duplicateGfxSeqnoEvent.opcode);
        }
    }
