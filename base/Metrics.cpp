    ...
    void operator()(const MetricEventVulkanOutOfMemory vkOutOfMemoryEvent) const {
        if (MetricsLogger::add_vulkan_out_of_memory_event) {
            MetricsLogger::add_vulkan_out_of_memory_event(
                ...
                vkOutOfMemoryEvent.opCode.has_value() ? vkOutOfMemoryEvent.opCode.value() : 0,
                vkOutOfMemoryEvent.function,
                vkOutOfMemoryEvent.line.has_value() ? vkOutOfMemoryEvent.line.value() : 0,
                vkOutOfMemoryEvent.allocationSize.has_value()
                    ? vkOutOfMemoryEvent.allocationSize.value()
                    : 0,
