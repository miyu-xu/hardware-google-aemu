...
AbortMessage::~AbortMessage() {
    ...
    absl::log_internal::LogMessage(mFile, mLine, customDieFunction ? absl::LogSeverity::kError : absl::LogSeverity::kFatal)
        << "FATAL error in " << mFunction << ",  GURU MEDITATION ERROR:" << mReason.getAbortCode();

    // Note, that this will not be reached if there is no custom die function.
    die();
}

void setDieFunction(std::optional<std::function<void()>> newDie) {
    customDieFunction = newDie;
}
}  // namespace emugl
