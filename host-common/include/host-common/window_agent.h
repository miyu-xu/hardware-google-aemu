...
typedef struct QAndroidEmulatorWindowAgent {
    ...
    bool (*setXrHeadRotationEvent)(float x,float y, float z);
    // Sets the XR head movement event.
    bool (*setXrHeadMovementEvent)(float delta_x, float delta_y, float delta_z);
    // Sets the XR head angular velocity event.
    bool (*setXrHeadAngularVelocityEvent)(float omega_x, float omega_y, float omega_z, float transition_time_sec);
    // Sets the XR head velocity event.
    bool (*setXrHeadVelocityEvent)(float x, float y, float z, float transition_time_sec);
    // Sets the XR options.
